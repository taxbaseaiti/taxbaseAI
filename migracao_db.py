...
# --- Índice para acelerar buscas na knowledge_base ---
cursor.execute("CREATE INDEX idx_knowledge_base_termo ON knowledge_base (termo);")
...
cursor.executemany(
    "INSERT INTO knowledge_base (termo, definicao, ferramenta_associada) VALUES (?, ?, ?)",
    dicionario_contabil
)
print("Base de Conhecimento populada com as ferramentas associadas.")

# --- Inserção de conceitos sem ferramenta associada (definições diretas) ---
conceitos_sem_ferramenta = [
    (
        'Receita vs Despesa',
        'Receita é o total das entradas financeiras de um período; Despesa corresponde às saídas financeiras necessárias para gerar receita. A diferença entre elas indica o lucro ou prejuízo do período.',
        None
    )
]
cursor.executemany(
...
print("Conceitos sem ferramenta associada adicionados à base de conhecimento.")
...
usuarios_iniciais = [
    ...
    (2, 'Utilizador Teste', 'user@email.com', user123, 'user')
]
cursor.executemany(
...
for empresa in empresas_para_carregar:
    try:
        cursor.execute("INSERT INTO empresas (id, nome) VALUES (?, ?)",
        ...
        for i in range(6):
            ...
            dre_df['valor']       = dre_df['valor'] * fator_variacao * (1 + (0.05 - 0.1 * np.random.rand(len(dre_df))))
...
cursor.executemany(
    "INSERT INTO permissoes (id_usuario, id_empresa) VALUES (?, ?)",
    permissoes_iniciais
)
